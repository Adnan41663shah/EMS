...
const Inquiries: React.FC = () => {
  ...
  // Filter state for sales users (all, pending, in_progress)
  const [attendanceFilter, setAttendanceFilter] = useState<'all' | 'pending' | 'in_progress'>('all');
  ...
  // Also filter out admitted students from all inquiry lists
  const { inquiries, allCount, pendingCount, inProgressCount } = useMemo(() => {
    // Filter out admitted students from all inquiries
    ...
    // Calculate counts first using the same logic
    // PENDING = NOT attended (not assigned)
    // IN PROGRESS = attended (assigned or has assignment status)
    ...
    // Filter inquiries based on attendance filter
  ...
  const getRowBg = (inq: Inquiry) => {
    ...
    // Forwarded to sales but still assigned to someone (shouldn't happen after fix, but kept for safety)
    if ((inq as any).assignmentStatus === 'forwarded_to_sales' && inq.assignedTo) {
    ...
    <div className="space-y-2">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {/* Filter Buttons for Sales Users */}
          {user?.role === 'sales' && (
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                onClick={() => setAttendanceFilter('in_progress')}
                className={cn(
                  'inline-flex items-center px-4 py-2 text-sm font-medium rounded-lg transition-colors',
